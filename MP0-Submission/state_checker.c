   ...
   char address_str[50];  // Adjust the size based on your needs
    ...
    for (i = 0; i < 480; i++) {
      for (j = 0; j < 640; j++) {
    	  ...
    	  if(row_counter == SQUARE_WIDTH){
    		 color_select = (color_select == 0)? 1 : 0;
    		  row_counter = 0;
    	  }
    	  if(col_counter == SQUARE_HEIGHT){
    		 color_select = (color_select == 0)? 1 : 0;
    		 col_counter = 0;
    	  }
    	  ...
    	  // Cardinal Red -0xC8103E    Red C8 1010 1000    200*15 + 135 >> 8 ->0x0C.  Green 16*15 + 135 >> 8 0x01 . 62*15 + 135 >> 8 0x04 .
    	  // Cyclone Gold - 0xF1BE48
    	  test_image[i][j] = (color_select == 0)? 0x041C:0x04CF ;
/*
        if (j < 213) {
